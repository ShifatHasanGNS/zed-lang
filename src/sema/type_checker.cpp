...
TypeRef TypeChecker::check_struct_lit(StructLitExpr* e) {
    ...
    if (!st) {
        // Unknown struct — if cimports are present treat as a C foreign type.
        // This also handles Shape_Data which is generated by codegen and not
        // registered as a Zed struct.
        ...
        // Even without cimports, allow unregistered types (generated inner unions)
        // by returning a foreign named type rather than erroring.
        return sym_.arena().make_foreign_named(e->type_name);
    }
...
void TypeChecker::check_match(MatchStmt* s) {
    ...
    // Detect tagged union: the match value's type is a struct with a corresponding
    // _Tag enum registered in the symbol table.
    bool is_tagged = false;
    sem::StructType* tagged_st = nullptr;
    if (vty && vty->is_struct()) {
        tagged_st = static_cast<sem::StructType*>(vty);
        // A tagged union has a corresponding <Name>_Tag enum
        is_tagged = (sym_.lookup_enum(tagged_st->name + "_Tag") != nullptr);
    }
    ...
    for (auto& mc : s->cases) {
        ...
        if (mc.value) {
            ...
            // Tagged union binding: case .Circle(c) { }
            // Inject `c` as the variant's payload type into the case scope.
            if (is_tagged && !mc.binding.empty() && tagged_st) {
                // Find the variant field type
                std::string variant_name;
                if (auto* id = dynamic_cast<IdentExpr*>(mc.value))
                    variant_name = id->name;
                TypeRef payload_ty = sym_.arena().ty_error();
                for (const auto& f : tagged_st->fields) {
                    if (f.name == variant_name) { payload_ty = f.type; break; }
                }
                Symbol binding_sym(Symbol::Kind::VAR, mc.binding,
                                   payload_ty, mc.loc);
                binding_sym.initialized = true;
                sym_.declare(binding_sym);
            }
        }
